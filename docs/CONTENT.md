...
**decorative theme asset** loaded from a CDN; the foreground **railing** and the **cat**
that walks toward your cursor (and sits when it reaches it) are drawn entirely in inline
SVG/CSS (no external files) — the cat is `#cat` and the rail is `#rail` in `index.html`,
styled under "Foreground railing" / "The cat follows the pointer" in `src/style.css` and
driven from `src/main.js`. These are decorative, so you can leave them as-is. To change
the poetic copy, edit the text inside `.panel-left__head`, `.panel-right__top`, and the `<h2>` in
`.sec2`.
